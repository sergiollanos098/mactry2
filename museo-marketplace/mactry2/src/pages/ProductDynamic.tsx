...
const ProductDynamic = () => {
  ...
  const [sliderRef, slider] = useKeenSlider<HTMLDivElement>({
    slides: { perView: 1, spacing: 12 }, // SOLO 1 en móvil
    breakpoints: {
      "(min-width: 640px)": { slides: { perView: 2, spacing: 16 } }, // 2 en tablet
      "(min-width: 1024px)": { slides: { perView: 4, spacing: 24 } }, // 4 en desktop
    },
    loop: true,
  });
  ...
  return (
    <div className="max-w-2xl md:max-w-4xl mx-auto mt-4 px-2">
      {/* Imagen y datos */}
      <div className="flex flex-col md:flex-row gap-8 items-start">
        <img
          src={product.imagen}
          alt={product.nombre}
          className="w-full max-w-xs md:w-80 h-80 object-cover rounded shadow mx-auto"
        />
        <div className="flex-1 w-full">
          <h1 className="text-2xl md:text-3xl font-bold mb-2">{product.nombre}</h1>
          <p className="text-base md:text-lg mb-4" style={{ whiteSpace: "pre-line" }}>
            {product.descripcion}
          </p>
          <p className="text-xl md:text-2xl font-semibold mb-4">S./{product.precio}</p>
      ...
      <div className="bg-gray-50 p-4 rounded shadow mt-8">
        ...
        <form onSubmit={handleReviewSubmit} className="mb-4 flex flex-col sm:flex-row gap-2">
          <input
            type="text"
      ...
      {sugeridos.length > 0 && (
        <div className="mt-12 w-full">
          <h2 className="text-xl font-semibold mb-4 text-center">También podría interesarte</h2>
          <div className="relative max-w-xs mx-auto sm:max-w-2xl md:max-w-4xl">
            <button
            ...
            <div ref={sliderRef} className="keen-slider">
              {sugeridos.map((prod) => (
                <div
                  key={prod.id}
                  className="keen-slider__slide flex flex-col items-center bg-white rounded shadow p-3"
                >
                  <img
                    src={prod.imagen}
                    alt={prod.nombre}
